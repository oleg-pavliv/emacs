...
(defun bubble-buffer-next()
  ...
  (interactive)
  ;; (if (and (not (eq last-command 'bubble-buffer-next)) (not (eq last-command 'kill-buffer-quitly)))
  (unless (or (eq last-command 'kill-buffer-quitly) (eq last-command 'bubble-buffer-next))
  ...
  (setq bubble-buffer-list (remove-if-not #'(lambda (buf) (get-buffer buf)) bubble-buffer-list))
  ;; (print (format "last-cmd %s\n" last-command) (get-buffer-create "bubble-buffer-buffers"))
  ;; (print bubble-buffer-list (get-buffer-create "bubble-buffer-buffers"))
